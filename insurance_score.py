...
def calculate_precision_at_1(predicted_answers, ground_truth_answers, qid_range):
    ...
    for gt in ground_truth_answers['ground_truths']:
        qid = gt['qid']
        # Only calculate for QIDs within the specified range (Insurance range is 1 to 50)
        if qid in qid_range:
...
if __name__ == "__main__":
    ...
    predicted_file_path = "dataset/preliminary/insurance_pred_retrieve.json"  # JSON file for predicted results
    ...
    # Set the QID range for the Insurance part (QIDs 1 to 50)
    insurance_qid_range = range(1, 51)

...
if __name__ == "__main__":
    ...
    corpus_dict_finance = load_data_parallel(source_path_finance)

    # 預先計算財務語料庫的嵌入向量
    logging.info("計算財務語料庫的嵌入向量...")
    corpus_embeddings = {}
    for chunk_id, chunk_text in tqdm(corpus_dict_finance.items(), desc="計算嵌入"):
    ...
    # 創建 Document 對象列表
    documents = []
    for chunk_id, text in corpus_dict_finance.items():
